...
<task type="auto">
  ...
  <action>
    ...
    - Handlers delegate directly to module instances
  ...
  <verify>grep -n "constant HANDLERS" pike-scripts/analyzer.pike</verify>
  <done>HANDLERS constant exists with all 12 method handlers</done>
</task>
...
<task type="auto">
  ...
  <verify>grep -n "protected mapping dispatch" pike-scripts/analyzer.pike</verify>
  <done>dispatch() function exists and handle_request() takes Context parameter</done>
</task>
...
3. Call dispatch() with "parse" method - verify parser delegation works
4. Call dispatch() with "introspect" method - verify intelligence delegation works
5. Call dispatch() with "find_occurrences" method - verify analysis delegation works
6. Call dispatch() with unknown method - verify -32601 error response
7. Verify all handlers receive Context parameter
</verification>

<success_criteria>
1. Context class exists with all module instances
2. HANDLERS constant has 12 method entries
3. dispatch() function performs O(1) method lookup
4. dispatch() returns -32601 for unknown methods
5. dispatch() wraps handler errors in -32000 responses
6. handle_request() delegates to dispatch()
7. All module singletons initialized once in Context.create()
</success_criteria>
