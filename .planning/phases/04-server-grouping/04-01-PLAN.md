...
# Source code - existing core files (errors.ts, logging.ts already exist)
...
<task type="auto">
  <name>Task 1: Create core/types.ts with shared types</name>
  <files>packages/pike-lsp-server/src/core/types.ts</files>
  <action>
Create `packages/pike-lsp-server/src/core/types.ts` with shared types extracted from server.ts:
...
DO NOT duplicate - check if these types are used elsewhere first.
  ...
  <verify>grep -q "export.*PikeSettings" packages/pike-lsp-server/src/core/types.ts && grep -q "export.*DocumentCacheEntry" packages/pike-lsp-server/src/core/types.ts</verify>
  <done>core/types.ts exports PikeSettings and DocumentCacheEntry interfaces</done>
</task>
...
<success_criteria>
1. core/types.ts exports shared types (PikeSettings, DocumentCacheEntry)
2. services/document-cache.ts encapsulates document cache logic
3. services/bridge-manager.ts wraps PikeBridge with health monitoring
4. services/index.ts exports Services interface and re-exports services
