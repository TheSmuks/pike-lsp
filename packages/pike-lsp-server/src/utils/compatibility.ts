...
export function parseVersion(versionString: string): PikeVersionInfo | null {
  ...
  return {
    ...
    string: versionString.includes('Pike') ? versionString : `Pike v${normalizedCore}${suffix}`,
  };
}
...
export function getCompatibilityInfo(
  ...
  const requiredVersion =
    typeof required === 'string' ? (parseVersion(required) ?? MIN_SUPPORTED_VERSION) : required;
...
export function handleMissingModule(moduleName: string): never {
  throw new Error(
    `Module '${moduleName}' is not available in this Pike version. This feature requires Pike 8.0 or higher. Check that your Pike installation includes this module and that PIKE_PATH is correctly configured.`
  );
}
...
export function detectFeatureForVersion(featureName: string, version: VersionLike): boolean {
...
export function createTrimWrapper(
  version: VersionLike
  ...
  return (value: string | null | undefined): string =>
    normalizeInput(value).replace(/^\s+|\s+$/gu, '');
}

export function trim(
  value: string | null | undefined,
  version: VersionLike = MIN_SUPPORTED_VERSION
): string {
