...
export function isPikePosition(obj: unknown): obj is PikePosition {
  ...
  return typeof pos['file'] === 'string' && typeof pos['line'] === 'number';
}
 ...
 */
export function validatePikeResponse<T>(
  obj: unknown,
  ...
  if (!typeGuard(obj)) {
    ...
    throw new Error(
      `Invalid Pike response: expected type '${typeName}', but received ${received}. This may indicate a version mismatch between the Pike bridge and the LSP server, or the Pike subprocess crashed. Received data: ${JSON.stringify(obj).slice(0, 200)}`
    );
  }
  return obj;
}
