...
function countOccurrencesExcludingDefinition(
  ...
  return tokens.filter((t: PikeToken) => t.text === symbolName && t.line !== definitionLine).length;
}
...
int main() {
    ...
    console.log(
      `  FIX: expectedCount=${expectedCount} (excludes definition at line ${definitionLine})`
    );
  });
...
int main() {
    ...
    const expectedCount = countOccurrencesExcludingDefinition(tokens, 'helper', definitionLine);
...
int main() {
    ...
    const expectedCount = countOccurrencesExcludingDefinition(tokens, 'global_var', definitionLine);
    ...
    console.log(
      `  FIX: expectedCount=${expectedCount} (excludes declaration at line ${definitionLine})`
    );
  });
...
int main() {
    ...
    const expectedCount = countOccurrencesExcludingDefinition(tokens, 'my_method', definitionLine);

    assert.equal(currentCount, 1, 'BUG: Counts method definition');
    assert.equal(expectedCount, 0, 'FIXED: Method has 0 references');

    console.log(
      `  BUG: currentCount=${currentCount} (includes definition at line ${definitionLine})`
    );
