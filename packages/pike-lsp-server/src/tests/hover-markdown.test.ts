...
describe('Hover Markdown Conversion', () => {
  ...
  it('should convert url tag @url{...@}', () => {
    assert.equal(
      convertPikeDocToMarkdown('See @url{http://pike.lysator.liu.se@}'),
      'See <http://pike.lysator.liu.se>'
    );
  });

  it('should convert rfc tag @rfc{...@}', () => {
    assert.equal(
      convertPikeDocToMarkdown('See @rfc{1234@}'),
      'See [RFC 1234](https://tools.ietf.org/html/rfc1234)'
    );
  });
...
@}`;
  ...
  describe('Stress Tests - URL and Link Variations', () => {
    it('should handle various URL formats', () => {
      assert.equal(convertPikeDocToMarkdown('@url{https://example.com@}'), '<https://example.com>');
      assert.equal(
    ...
    it('should handle @ref with various identifiers', () => {
      assert.equal(convertPikeDocToMarkdown('@ref{Stdio.File@}'), '`Stdio.File`');
      // Leading space is preserved in the content
      assert.equal(convertPikeDocToMarkdown('@ref{ Standards.JSON@}'), '` Standards.JSON`');
    });
  });
  ...
  describe('Stress Tests - Additional Autodoc Tags', () => {
    ...
    it('should handle @expr tag with complex expressions', () => {
      assert.equal(convertPikeDocToMarkdown('@expr{x + y * z@}'), '`x + y * z`');
    });

    it('should handle @code tag as alias for @tt', () => {
      assert.equal(convertPikeDocToMarkdown('@code{my_function@}'), '`my_function`');
    });
