...
// Pike source locations (override with PIKE_SOURCE_ROOT/PIKE_STDLIB/PIKE_TOOLS env vars)
...
const PIKE_TOOLS = process.env['PIKE_TOOLS'] ?? path.join(PIKE_SOURCE_ROOT, 'lib/include');
...
async function testCategory(
  ...
  console.log(
    `   → ${passed}/${files.length} passed (${((passed / files.length) * 100).toFixed(1)}%)`
  );
 ...
 */
function validateSymbolRanges(results: FileTestResult[]): {
  valid: number;
  invalid: number;
  examples: string[];
} {
...
async function main(): Promise<void> {
  ...
  for (const summary of summaries) {
    ...
    console.log(
      `    Files: ${summary.tested} | Passed: ${summary.passed} | Failed: ${summary.failed} | Rate: ${rate}%`
    );
    console.log(
      `    Symbols: ${summary.totalSymbols} | Avg Parse Time: ${summary.avgParseTimeMs}ms`
    );
  }
