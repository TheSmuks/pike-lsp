...
function findReferencesInText(text: string, word: string, uri: string): Location[] {
...
function findWordAtPosition(text: string, offset: number): string | null {
  let start = offset;
  let end = offset;
  ...
  describe('Scenario 7.2: Highlight none on whitespace', () => {
    it('should return empty highlights for whitespace', () => {
      ...
      assert.ok(
        ...
        'Should return empty or find adjacent word'
      );
    });
