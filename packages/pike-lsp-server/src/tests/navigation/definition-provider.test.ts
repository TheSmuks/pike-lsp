  ...
  if (!opts.noCache) {
    cacheEntries.set(
      ...
      makeCacheEntry({
        ...
        symbolPositions: opts.symbolPositions
          ? new Map(Array.from(opts.symbolPositions.entries()).map(([k, v]) => [k, v]))
          : new Map(),
...
describe('Definition Provider', () => {
      ...
      bridge: {
        ...
        engineQuery: async () => ({
          ...
          result: {
            locations: [
              {
                uri: 'file:///query-definition.pike',
                range: {
  ...
  it('keeps parity with fallback when query returns stub', async () => {
    ...
    const baseline = await withoutQuery.definition(1, 8);
    const stubbed = await withStubQuery.definition(1, 8);

    expect(stubbed).toEqual(baseline);
  });

  it('keeps definition handler p95 non-regressing with query stub path', async () => {
    const code = `int root = 1;\nint k = root;`;
    const symbols = [
      {
        name: 'root',
        kind: 'variable' as const,
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      await baselineSetup.definition(1, 8);
      baselineTimes.push(performance.now() - baselineStart);

      const queryStart = performance.now();
      await querySetup.definition(1, 8);
...
int x = obj->parentMethod();`;
        ...
        const { definition } = setup({
          ...
          inherits: [{ from: 'Parent', file: 'test.pike', line: 6 }],
        });
        ...
        const { definition } = setup({
          ...
          symbols: [
            ...
            },
            {
              ...
              kind: 'class',
              modifiers: [],
              position: { file: 'test.pike', line: 11 },
            },
          ],
          inherits: [
