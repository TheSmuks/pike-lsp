 ...
 */
function createMethod(name: string, argNames: string[], argTypes?: string[]): PikeSymbol {
  return {
    name,
    kind: 'method',
    modifiers: [],
    argNames,
    argTypes:
      argTypes?.map(t => ({ kind: t, type: t })) ??
      argNames.map(() => ({ kind: 'mixed', type: 'mixed' })),
...
describe('Inlay Hints Provider Stress Tests', () => {
  ...
  describe('1. Large File Handling', () => {
    ...
    it('should handle file with 1000+ lines and many calls', () => {
      const symbols: PikeSymbol[] = [createMethod('handler', ['id', 'data'], ['int', 'string'])];
...
process(1, 2.5, "test", 4);`;
      ...
      const symbols = [
        createMethod('process', ['a', 'b', 'c', 'd'], ['int', 'float', 'string', 'int']),
      ];
...
handle(({1,2,3}), (["a":1]), lambda(int x) { });`;
      ...
      const symbols = [
        createMethod('handle', ['arr', 'map', 'cb'], ['array', 'mapping', 'function']),
      ];
      const configWithTypes: MockInlayHintsConfig = { ...defaultConfig, typeHints: true };
      const hints = extractInlayHints(code, symbols, configWithTypes);
