...
`;
    ...
    const addFunc = allSymbols.find(
      (s: unknown) => (s as Record<string, unknown>)['name'] === 'add'
...
`;
    ...
    const result = await bridge.parse(code, 'test.pike');
    const allSymbols = flattenSymbols(result.symbols);
    const trimFunc = allSymbols.find(
      (s: unknown) => (s as Record<string, unknown>)['name'] === 'trim'
    ) as Record<string, unknown> | undefined;
...
`;
    ...
    const result = await bridge.parse(code, 'test.pike');
    const allSymbols = flattenSymbols(result.symbols);
    const createFunc = allSymbols.find(
      (s: unknown) => (s as Record<string, unknown>)['name'] === 'create'
    ) as Record<string, unknown> | undefined;
...
`;
    ...
    const result = await bridge.parse(code, 'test.pike');
    const allSymbols = flattenSymbols(result.symbols);
    const func = allSymbols.find(
      (s: unknown) => (s as Record<string, unknown>)['name'] === 'process_internal'
    ) as Record<string, unknown> | undefined;
...
`;
    ...
    const allSymbols = flattenSymbols(result.symbols);
    const func = allSymbols.find(
      (s: unknown) => (s as Record<string, unknown>)['name'] === 'is_valid'
    ) as Record<string, unknown> | undefined;
...
`;
    ...
    const allSymbols = flattenSymbols(result.symbols);
    const func = allSymbols.find(
      (s: unknown) => (s as Record<string, unknown>)['name'] === 'to_upper'
    ) as Record<string, unknown> | undefined;
...
`;
    ...
    const allSymbols = flattenSymbols(result.symbols);
    const func = allSymbols.find(
      (s: unknown) => (s as Record<string, unknown>)['name'] === 'process_data'
    ) as Record<string, unknown> | undefined;
...
`;
    ...
    const allSymbols = flattenSymbols(result.symbols);
    const func = allSymbols.find(
      (s: unknown) => (s as Record<string, unknown>)['name'] === 'get_value'
    ) as Record<string, unknown> | undefined;
...
`;
    ...
    const allSymbols = flattenSymbols(result.symbols);
    const func = allSymbols.find(
      (s: unknown) => (s as Record<string, unknown>)['name'] === 'broken_func'
    ) as Record<string, unknown> | undefined;
    ...
    assert.ok(
      result.symbols !== undefined,
      'Parse returned valid symbols despite malformed autodoc'
    );
  });
...
`;
    ...
    const allSymbols = flattenSymbols(result.symbols);
    const func = allSymbols.find(
      (s: unknown) => (s as Record<string, unknown>)['name'] === 'no_doc_func'
    ) as Record<string, unknown> | undefined;
    ...
    assert.ok(
      !func!['documentation'] ||
        Object.keys(func!['documentation'] as Record<string, unknown>).length === 0,
...
`;
    ...
    const allSymbols = flattenSymbols(result.symbols);
    const func = allSymbols.find(
      (s: unknown) => (s as Record<string, unknown>)['name'] === 'empty_doc_func'
    ) as Record<string, unknown> | undefined;
...
`;
    ...
    const allSymbols = flattenSymbols(result.symbols);
    const func = allSymbols.find(
      (s: unknown) => (s as Record<string, unknown>)['name'] === 'get_this'
    ) as Record<string, unknown> | undefined;
    ...
    assert.strictEqual(
      func!['name'],
      'get_this',
      'function name should be correct (not include this_program)'
    );
  });
...
`;
    ...
    const allSymbols = flattenSymbols(result.symbols);
    const func = allSymbols.find(
      (s: unknown) => (s as Record<string, unknown>)['name'] === 'create_instance'
    ) as Record<string, unknown> | undefined;
