...
`;
    ...
    assert.strictEqual(
      myMethod.kind,
      'method',
      'myMethod should be a method (code lens applicable)'
    );
    ...
    assert.strictEqual(
      myVariable.kind,
      'variable',
      'myVariable should be a variable (no code lens)'
    );
    ...
    assert.strictEqual(
      globalFunction.kind,
      'method',
      'globalFunction should be a method (code lens applicable)'
    );
    ...
    assert.strictEqual(
      globalCounter.kind,
      'variable',
      'globalCounter should be a variable (no code lens)'
    );
  });
...
`;
    ...
    for (const symbol of symbolsWithPositions) {
      assert.ok(
        typeof symbol.position!.line === 'number',
        `${symbol.name} should have numeric line`
      );
