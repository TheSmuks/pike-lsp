  ...
  it('should format single parameter correctly', () => {
    ...
    const symbol = createSymbol({
      ...
      type: {
        kind: 'function',
        returnType: { kind: 'int' },
        arguments: [{ type: { kind: 'int' }, name: 'x' }],
      },
      documentation: {
...
describe('Hover Formatting - Edge Cases', () => {
  it('should handle complex type signatures', () => {
    ...
    const symbol = createSymbol({
      ...
      type: {
        ...
        arguments: [{ type: { kind: 'array' }, name: 'items' }],
      },
      documentation: {
  ...
  it('should handle this_program return type', () => {
    ...
    assert.ok(
      content!.includes('this_program') || content!.includes('create'),
      'Should include type or name'
    );
  });
...
describe('Hover Formatting - Special Documentation', () => {
  ...
  it('should handle examples', () => {
    ...
    const symbol = createSymbol({
      ...
      documentation: {
        ...
        examples: ['int result = exampleFunction();'],
      },
    });
  ...
  it('should handle see also references', () => {
    ...
    assert.ok(
      content!.includes(
        '[`otherFunction`](https://pike.lysator.liu.se/generated/manual/modref/ex/otherFunction.html)'
      )
    );
    assert.ok(
      content!.includes(
        '[`String.trim`](https://pike.lysator.liu.se/generated/manual/modref/ex/predef_3A_3A/String/trim.html)'
      )
    );
    assert.ok(
      content!.includes(
        '[`Stdio.File`](https://pike.lysator.liu.se/generated/manual/modref/ex/predef_3A_3A/Stdio/File.html)'
      )
    );
  });
});
...
describe('Hover Formatting - Stdlib Links', () => {
  ...
  it('should NOT add documentation link for user symbols', () => {
    ...
    assert.ok(
      !content!.includes('[Online Documentation]') || content!.includes('predef_3A_3A') === false,
