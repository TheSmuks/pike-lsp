...
describe('Stdlib Hover Tests', () => {
  ...
  it('should respond in under 500ms for first stdlib hover', async () => {
    ...
    const { result, duration } = await measureResolve('Stdio', () => bridge.resolveStdlib('Stdio'));

    assert.ok(result.found !== 0, 'Stdio should be found');
    assert.ok(
      duration < 500,
      `First hover should respond in under 500ms, took ${duration.toFixed(2)}ms`
    );
  ...
  it('should include common stdlib functions', async () => {
    ...
    const foundCommon = commonFunctions.filter(name => symbolNames.some(s => s.includes(name)));
  ...
  it('should handle nested module resolution (Stdio.File)', async () => {
    ...
    assert.ok(
      result.module === 'Stdio.File' || result.module === 'File',
