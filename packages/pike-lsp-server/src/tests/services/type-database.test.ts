...
function createMockProgramInfo(
  uri: string,
  overrides?: Partial<Parameters<typeof TypeDatabase.prototype.setProgram>[0]>
): Parameters<typeof TypeDatabase.prototype.setProgram>[0] {
...
describe('TypeDatabase - 28.1 Cache program', () => {
  ...
  it('28.1.4 should estimate program size', () => {
    ...
    const inherits: InheritanceInfo[] = [{ path: '/path/to/parent.pike', name: 'ParentClass' }];
...
describe('TypeDatabase - 28.4 Type inheritance', () => {
  it('28.4.1 should track inheritance graph', () => {
    ...
    const programInfo = createMockProgramInfo('file:///test.pike', {
      ...
      inherits: [{ path: '/path/to/parent.pike', name: 'ParentClass' }],
    });
  ...
  it('28.4.2 should get inherited members for class', () => {
    ...
    const parentProgram = createMockProgramInfo('file:///parent.pike', {
      classes: new Map([['ParentClass', createMockSymbol('ParentClass', 'class')]]),
      symbols: new Map([['parentMethod', createMockSymbol('parentMethod', 'method')]]),
    });

    const childProgram = createMockProgramInfo('file:///child.pike', {
      classes: new Map([['ChildClass', createMockSymbol('ChildClass', 'class')]]),
      inherits: [{ path: '/path/to/parent.pike', name: 'ParentClass' }],
    });
  ...
  it('28.4.3 should handle multiple inheritance levels', () => {
    ...
    const grandparentProgram = createMockProgramInfo('file:///grandparent.pike', {
      classes: new Map([['Grandparent', createMockSymbol('Grandparent', 'class')]]),
    });

    // Parent inherits from Grandparent
    const parentProgram = createMockProgramInfo('file:///parent.pike', {
      classes: new Map([['Parent', createMockSymbol('Parent', 'class')]]),
      inherits: [{ path: '/path/to/grandparent.pike', name: 'Grandparent' }],
    });

    // Child inherits from Parent
    const childProgram = createMockProgramInfo('file:///child.pike', {
      classes: new Map([['Child', createMockSymbol('Child', 'class')]]),
      inherits: [{ path: '/path/to/parent.pike', name: 'Parent' }],
    });
  ...
  it('28.4.4 should handle class with no parents', () => {
    ...
    const db = new TypeDatabase();
    const programInfo = createMockProgramInfo('file:///test.pike', {
      classes: new Map([['OrphanClass', createMockSymbol('OrphanClass', 'class')]]),
      inherits: [],
    });
...
describe('TypeDatabase - 28.5 Cross-reference', () => {
  it('28.5.1 should index symbols globally', () => {
    ...
    const db = new TypeDatabase();
    const program1 = createMockProgramInfo('file:///test1.pike', {
      symbols: new Map([['sharedFunc', createMockSymbol('sharedFunc', 'function')]]),
    });
    const program2 = createMockProgramInfo('file:///test2.pike', {
      symbols: new Map([
  ...
  it('28.5.2 should handle symbol name collisions', () => {
    ...
    const db = new TypeDatabase();
    const program1 = createMockProgramInfo('file:///test1.pike', {
      symbols: new Map([['myFunc', createMockSymbol('myFunc', 'function')]]),
    });
    const program2 = createMockProgramInfo('file:///test2.pike', {
      symbols: new Map([['myFunc', createMockSymbol('myFunc', 'function')]]),
    });
