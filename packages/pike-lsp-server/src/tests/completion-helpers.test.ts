...
describe('Completion Helpers', () => {
  ...
  it('prioritizes types in type context', () => {
    ...
    assert.ok(
      ...
      'Class should have higher priority than variable in type context'
    );
    assert.ok(classItem.sortText!.startsWith('0_'), 'Class should have priority 0 in type context');
    assert.ok(
      varItem.sortText!.startsWith('9_'),
      'Variable should have priority 9 in type context'
    );
  });
  ...
  it('prioritizes values in expression context', () => {
    ...
    const classItem = buildCompletionItem(
      'MyClass',
      classSymbol,
      'source',
      undefined,
    ...
    assert.ok(
      ...
      'Variable should have higher priority than class in expression context'
    );
    assert.ok(
      varItem.sortText!.startsWith('0_'),
      'Variable should have priority 0 in expression context'
    );
    assert.ok(
      classItem.sortText!.startsWith('2_'),
      'Class should have priority 2 in expression context'
    );
  });
  ...
  it('handles constants appropriately', () => {
    ...
    const exprItem = buildCompletionItem(
      'MY_CONST',
      constSymbol,
      'source',
      undefined,
    ...
    assert.ok(
      typeItem.sortText!.startsWith('1_'),
      'Constant should have priority 1 in type context'
    );
    assert.ok(
      exprItem.sortText!.startsWith('1_'),
      'Constant should have priority 1 in expression context'
    );
  });
