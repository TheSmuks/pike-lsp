...
`;
    ...
    // Verify positions
    assert.ok(
      variableX.position && variableX.position.line > 0,
      'Variable x should have valid line'
    );
    assert.ok(
      variableName.position && variableName.position.line > 0,
      'Variable name should have valid line'
    );
