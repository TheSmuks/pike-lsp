...
`;
    ...
    assert.ok(invalidResult.diagnostics.length > 0, 'Invalid code should have errors');
    assert.ok(invalidResult.diagnostics[0], 'Should have at least one diagnostic');
    assert.strictEqual(invalidResult.diagnostics[0].severity, 'error', 'Should be error severity');
  });
...
`;
    ...
    assert.ok(
      allSymbols.some(s => s.name === 'HTTPResponse'),
      'Should find HTTPResponse'
    );
    assert.ok(
      allSymbols.some(s => s.name === 'status_code'),
      'Should find status_code'
    );
    assert.ok(
      allSymbols.some(s => s.name === 'set_status'),
      'Should find set_status method'
    );
    assert.ok(
      allSymbols.some(s => s.kind === 'inherit'),
      'Should find inherit statement'
    );
  });
...
describe('Batch Parse Performance (PERF-002)', () => {
  ...
  it('should parse multiple files in a single batch request', async () => {
    ...
    assert.ok(
      result.results[0]?.symbols.some(s => s.name === 'x'),
      'Should find x in test1.pike'
    );

    // Check second file
    assert.strictEqual(result.results[1]?.filename, 'test2.pike');
    assert.ok(
      result.results[1]?.symbols.some(s => s.name === 's'),
      'Should find s in test2.pike'
    );

    // Check third file
    assert.strictEqual(result.results[2]?.filename, 'test3.pike');
    assert.ok(
      result.results[2]?.symbols.some(s => s.name === 'Foo'),
      'Should find Foo class in test3.pike'
    );
  });
  ...
  it('should be faster than sequential parsing for multiple files', async () => {
    ...
    assert.ok(
      batchTime <= seqTime * 1.5,
      `Batch (${batchTime}ms) should not be significantly slower than sequential (${seqTime}ms)`
    );
  });
});
