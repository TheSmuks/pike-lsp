...
describe('L. Real Stdlib Resolution', () => {
  ...
  it('L.7: resolved symbols have type information', async () => {
    ...
    if (result.found && result.symbols && result.symbols.length > 0) {
      const funcSymbols = result.symbols.filter((s: IntrospectedSymbol) => s.kind === 'function');
...
`;
    ...
    if (result?.success) {
      ...
      const classSymbol = result.symbols.find((s: IntrospectedSymbol) => s.name === 'TestClass');
      if (classSymbol) {
...
`;
    ...
    if (result?.success) {
      ...
      if (symbolNames.includes('read')) {
        const readSym = result.symbols.find((s: IntrospectedSymbol) => s.name === 'read');
        if (readSym) {
...
`;
    ...
    if (result?.success) {
      ...
      const createFunc = result.functions?.find((f: IntrospectedSymbol) => f.name === 'create');
      if (createFunc) {
