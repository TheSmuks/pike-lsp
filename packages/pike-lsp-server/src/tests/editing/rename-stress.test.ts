...
function createMockConnection(): MockConnection {
  ...
  return {
    ...
    onRenameRequest(handler: RenameHandler) {
...
int y = MY_CONSTANT * 2;`;
      ...
      const { prepareRename, rename } = setup({
        code,
        symbols: [sym('MY_CONSTANT', 'constant', { value: '100' } as any)],
      });
...
string s = add("a", "b");`;
      ...
      const { prepareRename, rename } = setup({
        code,
        symbols: [sym('add', 'method', { isVariant: true } as any)],
      });
...
int result = Math::square(5);`;
    ...
    int y = helper(5);
      ...
      const { prepareRename, rename } = setup({
        code,
        symbols: [sym('helper', 'method', { scope: 'local' } as any)],
      });
...
class Derived {
    inherit Base;
      ...
      const { prepareRename, rename } = setup({
        code,
        symbols: [sym('Base', 'class'), sym('Derived', 'class')],
      });
...
void main() {
    sharedHelper();
      ...
      const { prepareRename, rename } = setup({
        code,
        symbols: [sym('sharedHelper', 'method')],
      });
...
void main() {
    externalFunc();
      ...
      const { prepareRename, rename } = setup({
        code,
        symbols: [sym('externalFunc', 'method', { isExtern: true } as any)],
      });
...
int x = MY_CONSTANT;`;
      ...
      const { prepareRename, rename } = setup({
        code,
        symbols: [sym('MY_CONSTANT', 'constant', { fromInclude: 'constants.pike' } as any)],
      });
...
Stdio.File f = Stdio.File();`;
      ...
      const { prepareRename, rename } = setup({
        code,
        symbols: [sym('Stdio', 'module'), sym('File', 'class', { parentModule: 'Stdio' } as any)],
      });
      ...
      try {
        ...
        const changedUris = new Set(
          (result?.documentChanges ?? [])
            .filter((change): change is TextDocumentEdit => 'textDocument' in change)
...
int x = ${veryLongName};`;
      ...
      const { prepareRename, rename } = setup({
        code,
        symbols: [sym(veryLongName, 'variable')],
      });
...
int myVar = 2;`;
      ...
      const { prepareRename, rename } = setup({
        code,
        symbols: [sym('existing', 'variable'), sym('myVar', 'variable')],
      });
  ...
  describe('7. Performance Stress Tests', () => {
    it('should handle 100+ symbol references', async () => {
      ...
      const symbols = Array(101)
        .fill(null)
        .map((_, i) => sym('targetVar', 'variable'));
...
int z = 3;`;
      ...
      const promises = [rename(0, 4, 'a'), rename(1, 4, 'b'), rename(2, 4, 'c')];
    ...
    it('should handle code with many includes', async () => {
      const includes = Array(20)
        .fill(null)
        .map((_, i) => `#include "include${i}.pike"`)
        .join('\n');
      const code = `${includes}
int target = 0;
target++;`;
...
processCallback(5, lambda(int x) { return x * 2; });`;
      ...
      const { prepareRename, rename } = setup({
        code,
        symbols: [sym('processCallback', 'method')],
      });
    ...
    return result;
      ...
      const { prepareRename, rename } = setup({
        code,
        symbols: [sym('calculate', 'method'), sym('result', 'variable')],
      });
...
print(greeting);`;
      ...
      const { prepareRename, rename } = setup({
        code,
        symbols: [sym('name', 'variable'), sym('greeting', 'variable')],
      });
