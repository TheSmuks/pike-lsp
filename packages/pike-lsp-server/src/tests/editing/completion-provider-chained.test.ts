...
function createMockConnection(): MockConnection {
  ...
  return {
    onCompletion: (cb: CompletionHandler) => {
      h = cb;
    },
    onCompletionResolve: (cb: (item: CompletionItem) => CompletionItem) => {
      r = cb;
    },
...
function setup(opts: SetupOptions) {
  ...
  return {
    complete: (line: number, character: number) =>
      conn.completionHandler({ textDocument: { uri }, position: { line, character } }),
    ...
    it('D.3: type from function return value (chained access)', async () => {
      ...
      const { complete } = setup({
        ...
        symbols: [
          {
            name: 'getFile',
            type: { kind: 'function', returnType: { kind: 'object', className: 'Stdio.File' } },
            kind: 'function',
            modifiers: [],
          },
        ],
