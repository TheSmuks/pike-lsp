...
import { TypeHierarchyItem, Range, DiagnosticSeverity } from 'vscode-languageserver/node.js';
...
class Derived2 {
    inherit Base;
      ...
      assert.strictEqual(
        crossFileSubtypes.derivedFiles.length,
        2,
        'Should find subtypes in 2 files'
      );
...
class Child2 {
    ...
    void c2Method() { }
      ...
      assert.strictEqual(
        hierarchy.GrandParent.directSubtypes.length,
        1,
        'GrandParent has 1 direct subtype'
      );
      assert.strictEqual(
        hierarchy.GrandParent.directSubtypes[0],
        'Parent',
        'GrandParent subtype is Parent'
      );
      assert.strictEqual(hierarchy.Parent.directSubtypes.length, 2, 'Parent has 2 direct subtypes');
      assert.ok(
        hierarchy.Parent.directSubtypes.includes('Child1'),
        'Parent subtype includes Child1'
      );
      assert.ok(
        hierarchy.Parent.directSubtypes.includes('Child2'),
        'Parent subtype includes Child2'
      );
    });
...
class Derived {
    ...
    // Ambiguous: commonMethod exists in both
      ...
      assert.strictEqual(
        nameCollision.inheritedMethods.commonMethod.length,
        2,
        'commonMethod exists in 2 bases'
      );
      assert.ok(
        nameCollision.inheritedMethods.commonMethod.includes('Base1'),
        'commonMethod from Base1'
      );
      assert.ok(
        nameCollision.inheritedMethods.commonMethod.includes('Base2'),
        'commonMethod from Base2'
      );
    });
  });
...
class C { inherit A; }  // cycle: A->B->C->A`;
      ...
      assert.strictEqual(
        cycleDetection.cyclePath[cycleDetection.cyclePath.length - 1],
        'A',
        'Cycle ends at A'
      );
    });
...
module DerivedModule {
    inherit BaseModule;
      ...
      assert.ok(
        moduleHierarchy.inheritance.DerivedModule.includes('BaseModule'),
        'Module inheritance works'
      );
    });
...
class Implementation {
    ...
    void requiredMethod() {
      ...
      assert.ok(
        protocolPattern.requiredMethods.includes('requiredMethod'),
        'Required method declared'
      );
    });
  ...
  describe('Performance', () => {
    it('should recursively resolve supertypes in a deep inheritance chain', async () => {
      ...
      const services = createMockServices({
        cacheEntries: new Map<string, DocumentCacheEntry>([[uri, makeCacheEntry({ symbols })]]),
      });
...
enum DerivedEnum {
    ...
    C
      ...
      assert.strictEqual(
        enumHandling.supportsEnumInheritance,
        false,
        'Pike enums may not support inheritance'
      );
    });
  });
...
class B { inherit A; }`;
      ...
      assert.strictEqual(
        circularHandling.severity,
        DiagnosticSeverity.Warning,
        'Warning severity for cycles'
      );
    });
  });
