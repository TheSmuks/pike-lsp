...
void caller2() {
    helper();
      ...
      const expectedIncomingCalls: CallHierarchyIncomingCall[] = [
        {
          ...
          fromRanges: [{ start: { line: 2, character: 4 }, end: { line: 2, character: 12 } }],
        },
        {
          from: {
            name: 'caller2',
          ...
          fromRanges: [{ start: { line: 5, character: 4 }, end: { line: 5, character: 12 } }],
        },
      ];
      ...
      assert.strictEqual(
        expectedIncomingCalls[1]!.from.name,
        'caller2',
        'Second caller is caller2'
      );

      // Verify fromRanges structure
      assert.strictEqual(
        expectedIncomingCalls[0]!.fromRanges.length,
        1,
        'caller1 has one call site'
      );
      assert.strictEqual(
        expectedIncomingCalls[0]!.fromRanges[0]!.start.line,
        2,
        'Call site on line 2'
      );
    });
...
void caller2() {
    helper();
      ...
      assert.ok(
        crossFileCalls.some(c => c.uri.includes('caller1')),
        'Should include caller1.pike'
      );
      assert.ok(
        crossFileCalls.some(c => c.uri.includes('caller2')),
        'Should include caller2.pike'
      );
    });
...
void root() {
    ...
    caller2();
      ...
      assert.strictEqual(
        diamondPattern.shared.incomingCalls.length,
        2,
        'Shared has 2 incoming callers'
      );
...
`;
  ...
  describe('Scenario 13.4: Call Hierarchy - Cross-file calls', () => {
    it('should show outgoing calls to other files', async () => {
      ...
      const { createMockDocuments, createMockServices, makeCacheEntry, sym } =
        await import('../helpers/mock-services.js');
...
void caller() {
    helper();
      ...
      const conn = {
        languages: {
          callHierarchy: {
            ...
            onOutgoingCalls: (h: any) => {
              outgoingCallsHandler = h;
            },
            onIncomingCalls: () => {},
          },
          typeHierarchy: {
      ...
      assert.strictEqual(
        outgoingCalls[0].fromRanges[0].start.line,
        2,
        'Call should be on line 2 (0-indexed)'
      );
    });
...
void caller() {
    includedFunction();
      ...
      assert.strictEqual(
        includeCall.calls[0]!.function,
        'includedFunction',
        'Calls included function'
      );
    });
