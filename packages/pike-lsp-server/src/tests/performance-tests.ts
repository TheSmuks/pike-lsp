...
function measurePerformance<T>(
  ...
  return fn().then(result => {
    ...
    console.log(
      `  [PERF] ${operation}: ${duration.toFixed(2)}ms (${symbolsPerSecond.toFixed(0)} symbols/sec)`
    );
    return result;
  });
}
...
describe('Performance Tests - Large File Parsing', () => {
  ...
  it('should parse moderately sized file efficiently (100+ lines)', async () => {
    ...
    assert.ok(
      totalSymbols >= 50,
      `Should extract at least 50 symbols from 100 lines (got ${totalSymbols})`
    );
  });
  ...
  it('should handle multiple sequential parses efficiently', async () => {
    ...
    console.log(
      `  [PERF] Parse 3 files (300 lines): ${duration.toFixed(2)}ms (${avgSymbolsPerSecond.toFixed(0)} symbols/sec)`
    );
...
describe('Performance Tests - Memory and Resource Usage', () => {
  ...
  it('should handle rapid sequential parse operations', async () => {
    ...
    console.log(
      `  [PERF] ${iterations} parses: ${duration.toFixed(2)}ms (avg ${avgDuration.toFixed(2)}ms each)`
    );
    ...
    assert.ok(
      results.every(r => r.symbols.length > 0),
      'All parses should extract symbols'
    );
  });
...
describe('Performance Tests - Real-World Scenarios', () => {
  ...
  it('should simulate working with a medium project (10 files)', async () => {
    const files: string[] = [];
    for (let i = 0; i < 10; i++) {
      files.push(generatePikeCode(50 + Math.floor(Math.random() * 100), `ProjectModule${i}`));
    }
    ...
    console.log(
      `  [PERF] Total symbols: ${totalSymbols} (avg ${avgSymbolsPerFile.toFixed(0)} per file)`
    );
...
`;
    ...
    console.log(
      `  [PERF] Initial parse: ${initialDuration.toFixed(2)}ms (${initial.symbols.length} symbols)`
    );
    console.log(
      `  [PERF] Edit reparse: ${editDuration.toFixed(2)}ms (${edited.symbols.length} symbols)`
    );

    assert.ok(
      edited.symbols.length >= initial.symbols.length,
      'Reparse should find at least as many symbols'
    );
  });
});
