...
export function registerHierarchyHandlers(
  ...
  connection.languages.callHierarchy.onPrepare(params => {
    ...
    try {
      ...
      log.error('Call hierarchy prepare failed', {
  ...
  connection.languages.callHierarchy.onIncomingCalls(params => {
    ...
    try {
      ...
      log.error('Call hierarchy incoming calls failed', {
        error: err instanceof Error ? err.message : String(err),
      });
      return [];
    }
  ...
  connection.languages.callHierarchy.onOutgoingCalls(params => {
    ...
    try {
      ...
      log.error('Call hierarchy outgoing calls failed', {
        error: err instanceof Error ? err.message : String(err),
      });
      return [];
    }
  ...
  connection.languages.typeHierarchy.onPrepare(params => {
    ...
    try {
      ...
      log.error('Type hierarchy prepare failed', {
        error: err instanceof Error ? err.message : String(err),
      });
      return null;
    }
    ...
    try {
      ...
      while (queue.length > 0) {
        ...
        for (const symbol of currentCached.symbols) {
          ...
          const parentSymbol = currentCached.symbols.find(
            s => s.name === inheritedName && s.kind === 'class'
          );
          ...
          if (parentSymbol?.position) {
            ...
            for (const [otherUri, otherCached] of documentCache.entries()) {
              ...
              const parentInOther = otherCached.symbols.find(
                s => s.name === inheritedName && s.kind === 'class'
              );
      ...
      log.error('Type hierarchy supertypes failed', {
        error: err instanceof Error ? err.message : String(err),
      });
  ...
  connection.languages.typeHierarchy.onSubtypes(async params => {
    ...
    try {
      ...
      log.error('Type hierarchy subtypes failed', {
        error: err instanceof Error ? err.message : String(err),
      });
      // Publish error diagnostic with code
      connection.sendDiagnostics({
