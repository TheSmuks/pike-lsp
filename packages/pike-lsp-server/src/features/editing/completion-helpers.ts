...
export function buildCompletionItem(
  ...
  if (context === 'type') {
    ...
    if (
      ...
      priority = '1';
    } else if (kind === CompletionItemKind.Class || kind === CompletionItemKind.Module) {
  ...
  if (isFunction && context === 'expression') {
    const snippetInfo = buildMethodSnippet(
      name,
      symbol.type,
      symbolAny['argNames'] as (string | null)[] | undefined,
      symbolAny['argTypes'] as unknown[] | undefined
    );
...
export function buildLabelDetails(symbol: PikeSymbol): LabelDetails | undefined {
  ...
  if (symbol.kind === 'method') {
    ...
    const detail =
      params && returnType ? `${params}: ${returnType}` : params || returnType || undefined;
