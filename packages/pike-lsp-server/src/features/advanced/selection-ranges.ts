...
import { Connection, SelectionRange, Position } from 'vscode-languageserver/node.js';
...
function buildSymbolHierarchy(
  ...
  if (symbol.selectionRange) {
    ...
    if (
      wordBoundary.start.line === symbol.selectionRange.start.line &&
...
export function registerSelectionRangesHandler(
  ...
  connection.onSelectionRanges(async (params): Promise<SelectionRange[] | null> => {
    ...
    try {
      ...
      log.error(
        `Selection ranges failed for ${params.textDocument.uri}: ${err instanceof Error ? err.message : String(err)}`
      );
      return null;
    }
  });
