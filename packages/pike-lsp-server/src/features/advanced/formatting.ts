...
import { Connection, TextEdit, ResponseError, ErrorCodes } from 'vscode-languageserver/node.js';
...
export function registerFormattingHandlers(
  ...
  connection.onDocumentFormatting((params): TextEdit[] => {
    ...
    try {
      ...
      log.error('Document formatting failed', {
  ...
  connection.onDocumentRangeFormatting((params): TextEdit[] => {
    ...
    try {
      ...
      log.error('Range formatting failed', {
        error: err instanceof Error ? err.message : String(err),
      });
      throw new ResponseError(
        ErrorCodes.InternalError,
