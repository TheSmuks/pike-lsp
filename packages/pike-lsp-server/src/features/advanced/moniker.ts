...
export function registerMonikerHandler(
  ...
  connection.languages.moniker.on(async (params: MonikerParams): Promise<Moniker[] | null> => {
    ...
    try {
      ...
      log.error(
        `Moniker request failed for ${params.textDocument.uri}: ${err instanceof Error ? err.message : String(err)}`
      );
      return null;
    }
  });
  ...
  connection.onRequest('$/cancelRequest', async (params: { id: number | string }) => {
    ...
    try {
      if (services.bridge) {
        await services.bridge.engineCancelRequest({ requestId });
      }
    } catch (err) {
      log.debug('Cancel request forwarding failed', {
        requestId,
