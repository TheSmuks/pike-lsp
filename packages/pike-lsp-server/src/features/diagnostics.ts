...
export function registerDiagnosticsHandlers(
   ...
   */
  function buildSymbolPositionIndexRegex(
    text: string,
    symbols: PikeSymbol[]
  ): Map<string, Position[]> {
    const index = new Map<string, Position[]>();
  ...
  function validateDocumentDebounced(document: TextDocument): void {
    ...
    connection.console.log(
      `[DID_CHANGE] uri=${uri}, version=${version}, delay=${globalSettings.diagnosticDelay}ms`
    );
    ...
    const timer = setTimeout(() => {
      ...
      connection.console.log(
        `[DEBOUNCE] uri=${uri}, version=${version}, executing validateDocument`
      );
  ...
  async function validateDocument(
    ...
    try {
      ...
      connection.console.log(
        `[VALIDATE] Analyze completed - parse: ${hasParse}, introspect: ${hasIntrospect}, diagnostics: ${hasDiagnostics}`
      );
      ...
      if (analyzeResult.failures && Object.keys(analyzeResult.failures).length > 0) {
        connection.console.log(
          `[VALIDATE] Partial failures: ${Object.keys(analyzeResult.failures).join(', ')}`
        );
      }
      ...
      if (introspectData.success && introspectData.symbols.length > 0) {
        ...
        connection.console.log(
          `[VALIDATE] Introspection: success=${introspectData.success}, symbols=${introspectData.symbols.length}, functions=${introspectData.functions?.length || 0}, classes=${introspectData.classes?.length || 0}`
        );
        ...
        for (let i = 0; i < Math.min(5, introspectData.symbols.length); i++) {
          ...
          if (sym) {
            connection.console.log(
              `[VALIDATE]   Introspect ${i}: name="${sym.name}", kind=${sym.kind}`
            );
          }
        }
        ...
        if (parseData && parseData.symbols.length > 0) {
          ...
          connection.console.log(
            `[VALIDATE] Flattened ${parseData.symbols.length} symbols to ${flatParseSymbols.length} total (including class members)`
          );
          ...
          for (let i = 0; i < Math.min(5, flatParseSymbols.length); i++) {
            ...
            if (sym) {
              connection.console.log(
                `[VALIDATE]   Parsed ${i}: name="${sym.name}", kind=${sym.kind}`
              );
            }
          }
          ...
          for (const introspectedSym of introspectData.symbols) {
            ...
            if (!inParse) {
              connection.console.log(
                `[VALIDATE]   Adding introspected-only symbol: name="${introspectedSym.name}", kind=${introspectedSym.kind}`
              );
              legacySymbols.push({
        ...
        connection.console.log(
          `[VALIDATE] Cached document - total symbols: ${legacySymbols.length}, introspection success: ${introspectData.success}`
        );
        ...
        connection.console.log(
          `[VALIDATE] Using parse result with ${symbolsWithDeprecated.length} symbols (with deprecated extraction)`
        );
        ...
        for (let i = 0; i < Math.min(5, parseData.symbols.length); i++) {
          ...
          if (sym) {
            connection.console.log(
              `[VALIDATE]   Symbol ${i}: name="${sym.name}", kind=${sym.kind}`
            );
          }
        }
        // Resolve include/import dependencies for IntelliSense
        ...
        documentCache.set(uri, cacheEntry);
        connection.console.log(
          `[VALIDATE] Cached document - symbols count: ${symbolsWithDeprecated.length}`
        );
      } else {
      ...
      if (diagnosticsData.diagnostics && diagnosticsData.diagnostics.length > 0) {
        connection.console.log(
          `[VALIDATE] Found ${diagnosticsData.diagnostics.length} diagnostics from analyze`
        );
