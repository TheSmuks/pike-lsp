...
describe('CompilationCache.evictAndSerialize', () => {
  it('returns serialized survivors and evicted URIs in a single pass', () => {
    const cache = new CompilationCache<TestResult>(makeOptions({ clock: () => 1000 }));
    cache.store('file:///old.pike', 'old-code', { errors: 1 }, [], 500);
    cache.store('file:///new.pike', 'new-code', { errors: 0 }, [], 900);

    const { serialized, evicted } = cache.evictAndSerialize(300, 1000);
    ...
    assert.strictEqual(parsed.entries[0].uri, 'file:///new.pike');
  });

  it('returns empty evicted array and full serialization when nothing is expired', () => {
    const cache = new CompilationCache<TestResult>(makeOptions({ clock: () => 1000 }));
    cache.store('file:///a.pike', 'code-a', { errors: 0 }, [], 900);

    const { serialized, evicted } = cache.evictAndSerialize(200, 1000);
  ...
  it('returns empty serialization and all URIs evicted when everything is expired', () => {
    const cache = new CompilationCache<TestResult>(makeOptions({ clock: () => 1000 }));
    cache.store('file:///a.pike', 'code-a', { errors: 0 }, [], 100);
    cache.store('file:///b.pike', 'code-b', { errors: 0 }, [], 200);

    const { serialized, evicted } = cache.evictAndSerialize(100, 1000);
  ...
  it('cleans up dependency edges for evicted entries', () => {
    const cache = new CompilationCache<TestResult>(makeOptions({ clock: () => 1000 }));
    cache.store('file:///a.pike', 'code-a', { errors: 0 }, ['file:///b.pike'], 500);
    cache.store('file:///b.pike', 'code-b', { errors: 0 }, [], 900);

    cache.evictAndSerialize(300, 1000);
