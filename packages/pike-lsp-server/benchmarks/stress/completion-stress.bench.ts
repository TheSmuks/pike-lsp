...
function createMockConnection(): MockConnection {
  ...
  return {
    ...
    onCompletionResolve(handler: CompletionResolveHandler) {
...
describe('Completion Provider Stress Tests', () => {
  ...
  describe('1. Type-Based Completions', () => {
    it('should complete object type members from stdlib', async () => {
      ...
      fileSymbols.set('write', {
        name: 'write',
        type: { kind: 'function', returnType: { kind: 'int' } },
        kind: 'function',
        modifiers: ['public'],
      });
    ...
    it('should complete with complex generic types', async () => {
      ...
      arraySymbols.set('pop', {
        name: 'pop',
        type: { kind: 'function', returnType: { kind: 'mixed' } },
        kind: 'function',
        modifiers: [],
      });
    ...
    it('should resolve type from typedef', async () => {
      const { complete } = setup({
        ...
        symbols: [
          ...
          sym('val', 'variable', {
            type: { kind: 'typedef', name: 'MyTypedef', actualType: { kind: 'string' } } as any,
          }),
        ],
        bridgeContext: {
          context: 'member_access',
  ...
  describe('3. Snippet Completions', () => {
    ...
    it('should handle functions with optional parameters', async () => {
      const { complete } = setup({
        code: 'int x = ',
        symbols: [
          method('query', [{ name: 'table' }, { name: 'filter', type: 'mapping' }], 'array'),
        ],
      });
    ...
    it('should NOT generate snippets in type context', async () => {
      const { complete } = setup({
        code: '',
        symbols: [method('myFunction', [{ name: 'x' }])],
      });
  ...
  describe('4. Context-Aware Completions', () => {
    it('should prioritize types after inheritance', async () => {
      const { complete } = setup({
        code: 'inherit ',
        symbols: [sym('Stdio', 'module'), sym('MyClass', 'class'), sym('my_var', 'variable')],
      });
    ...
    void method() {
      ...
      const { complete } = setup({
        code,
        symbols: [classSym('MyClass', [sym('value', 'variable'), method('method', [])])],
      });
    ...
    it('should handle import context', async () => {
      const { complete } = setup({
        code: 'import ',
        symbols: [sym('Stdio', 'module'), sym('Array', 'module')],
      });
    ...
    it('should handle enum context', async () => {
      const { complete } = setup({
        code: 'enum { ',
        symbols: [sym('VALUE_A', 'constant'), sym('VALUE_B', 'constant')],
      });
  ...
  describe('5. Edge Cases: Generics and Inheritance', () => {
    it('should handle generic array<Type> syntax', async () => {
      const { complete } = setup({
        ...
        symbols: [
          sym('strings', 'variable', {
            type: { kind: 'array', elementType: { kind: 'string' } } as any,
          }),
        ],
        bridgeContext: {
          context: 'member_access',
    ...
    it('should handle generic mapping<Key, Value> syntax', async () => {
      const { complete } = setup({
        ...
        symbols: [sym('counts', 'variable', { type: { kind: 'mapping' } as any })],
        bridgeContext: {
          context: 'member_access',
          objectName: 'counts',
          prefix: '',
    ...
    it('should handle interface/implement completions', async () => {
      ...
      const { complete } = setup({
        code: 'implement ',
        symbols: [sym('MyInterface', 'class', { isInterface: true } as any)],
      });
  ...
  describe('6. Performance Stress Tests', () => {
    it('should handle 100+ symbols efficiently', async () => {
      const manySymbols = Array.from({ length: 150 }, (_, i) => sym(`symbol_${i}`, 'variable'));
    ...
    it('should handle 500+ symbols without timeout', async () => {
      const manySymbols = Array.from({ length: 500 }, (_, i) => sym(`var_${i}`, 'variable'));
    ...
    it('should handle rapid consecutive completions', async () => {
      ...
      const promises = [complete(0, 0), complete(0, 8), complete(1, 0), complete(1, 10)];
    ...
    it('should handle completion with many includes', async () => {
      const includeSymbols = Array.from({ length: 20 }, (_, i) => ({
        ...
        symbols: [sym(`func_${i}`, 'method'), sym(`const_${i}`, 'constant')],
      }));
  ...
  describe('7. Error Handling Stress Tests', () => {
    ...
    it('should handle special characters in code', async () => {
      const { complete } = setup({
        code: 'string s = "test\\ntest";\n',
        symbols: [sym('s', 'variable', { type: { kind: 'string' } as any })],
      });
