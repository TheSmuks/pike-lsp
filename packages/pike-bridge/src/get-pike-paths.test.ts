...
describe('getPikePaths', () => {
  ...
  it('should return paths from Pike runtime, not hardcoded fallbacks', async () => {
    ...
    const hasIncludeDir = result.include_paths.some((path: string) => path.endsWith('/include'));
    ...
    const hasModulesDir = result.module_paths.some((path: string) => path.endsWith('/modules'));
  ...
  it('should return paths that exist on the filesystem', async () => {
    ...
    result.include_paths.forEach((path: string) => {
      assert.ok(fs.existsSync(path), `Include path should exist on filesystem: ${path}`);
    });

    // Verify all module paths exist
    result.module_paths.forEach((path: string) => {
      assert.ok(fs.existsSync(path), `Module path should exist on filesystem: ${path}`);
    });
  });
  ...
  it('should return at least one include path and one module path', async () => {
    ...
    assert.ok(result.include_paths.length > 0, 'Should return at least one include path');

    assert.ok(result.module_paths.length > 0, 'Should return at least one module path');
  });
});
