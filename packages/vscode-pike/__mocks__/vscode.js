...
class MockRange {
  ...
  intersection(range) {
    const start =
      this.start.line > range.start.line
        ? this.start
        : this.start.line === range.start.line
          ? new MockPosition(this.start.line, Math.max(this.start.character, range.start.character))
          : range.start;
    const end =
      this.end.line < range.end.line
        ? this.end
        : this.end.line === range.end.line
          ? new MockPosition(this.end.line, Math.min(this.end.character, range.end.character))
          : range.end;
  ...
  with(change) {
    return new MockRange(change?.start ?? this.start, change?.end ?? this.end);
  }
}
...
class MockSelection extends MockRange {
  constructor(anchorLine, anchorCharacter, activeLine, activeCharacter) {
    ...
    this.isReversed =
      activeLine < anchorLine || (activeLine === anchorLine && activeCharacter < anchorCharacter);
  }
}
...
const mockWorkspaceFolder = {
  uri: new MockUri('file:///workspace/packages/vscode-pike/test-workspace'),
  name: 'test-workspace',
  index: 0,
};
...
const mockWorkspace = {
  ...
  fs: {
    ...
    stat: uri =>
      Promise.resolve({ isDirectory: () => false, isFile: () => true, size: 0, mtime: new Date() }),
...
const mockWindow = {
  ...
  createWebviewPanel: (viewType, title, showColumn, options) => ({
    webview: {
      html: '',
      options: {},
      onDidChangeMessage: mockEventEmitter.event,
