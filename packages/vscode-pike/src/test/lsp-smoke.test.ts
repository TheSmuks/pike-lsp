...
describe('LSP Smoke Test', function () {
  ...
  it('should start LSP server without Stdio timeout error', async function () {
    ...
    try {
      ...
      const hasTimeoutError = logs.some(
        log =>
      ...
      const hasStartupMessage = logs.some(
        l => l.includes('Pike LSP Server initialized') || l.includes('Pike bridge started')
      );
  ...
  it('should capture LSP version in logs', function () {
    ...
    console.log(
      'Version log:',
      logs.find(l => l.includes('Pike LSP Analyzer running'))
    );
  ...
  it('should not have compilation errors', function () {
    ...
    const logs = mockOutputChannel.getLogs();
    const hasCompilationError = logs.some(
      l => l.includes('Compilation failed') || l.includes('PikeCompiler')
    );
    ...
    if (hasCompilationError) {
      ...
      logs
        .filter(
          l => l.includes('Compilation') || l.includes('Undefined') || l.includes('PikeCompiler')
        )
        .forEach(l => console.error('  ', l));
