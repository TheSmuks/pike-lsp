...
suite('Performance Benchmark Tests', () => {
  ...
  test('47.1 Large file parsing completes within time limit', async function () {
    ...
    try {
      ...
      assert.ok(
        elapsed < 10000,
        `Large file parsing should complete within 10 seconds (took ${elapsed}ms)`
      );
  ...
  test('47.2 Workspace symbol search completes within time limit', async function () {
    ...
    assert.ok(
      elapsed < 30000,
      `Workspace search should complete within 30 seconds (took ${elapsed}ms)`
    );

    console.log(
      `Workspace search completed in ${elapsed}ms, found ${symbols?.length || 0} symbols`
    );
  });
  ...
  test('47.4 Find references response time within threshold', async function () {
    ...
    if (varMatch) {
      ...
      assert.ok(
        elapsed < 5000,
        `Find references should complete within 5 seconds (took ${elapsed}ms)`
      );

      console.log(
        `Find references completed in ${elapsed}ms, found ${references!.length} references`
      );
    } else {
  ...
  test('47.5 Workspace search response time within threshold', async function () {
    ...
    assert.ok(
      elapsed < 10000,
      `Workspace search should complete within 10 seconds (took ${elapsed}ms)`
    );
