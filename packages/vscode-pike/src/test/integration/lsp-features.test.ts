...
suite('LSP Feature E2E Tests', () => {
      ...
      const diagnosticListener = vscode.languages.onDidChangeDiagnostics(e => {
        for (const uri of e.uris) {
          ...
          diagnostics.forEach(d => {
            logServerOutput(
              `Diagnostic: ${d.severity} - ${d.message} at line ${d.range.start.line}`
            );
          });
        }
      });
  ...
  test('Document symbols returns valid symbol tree', async function () {
    ...
    const hasTopLevelSymbols = symbols.some(
      s =>
    ...
    assert.ok(
      hasTopLevelSymbols || symbols.length > 0,
...
`;
    ...
    const hasNormalSymbol = symbols?.some(
      s => s.name === 'normal_symbol' || s.name === 'NormalClass'
    );
    ...
    const hasPlatformSymbol = symbols?.some(
      s => s.name === 'platform_specific_symbol' || s.name === 'PlatformClass'
    );
    const hasDisabledSymbol = symbols?.some(
      s => s.name === 'disabled_symbol' || s.name === 'DisabledClass'
    );

    assert.ok(
      hasPlatformSymbol || hasDisabledSymbol,
    ...
    const hasSplitBlockSymbol = symbols?.some(
      s => s.name === 'debug_function' || s.name === 'release_function'
    );
    ...
    const hasPlatformVariant = symbols?.some(
      s => s.name === 'platform_a_var' || s.name === 'platform_b_var' || s.name === 'generic_var'
    );
  ...
  test('Go-to-definition returns location', async function () {
    ...
    assert.ok(
      locations,
      'Should return definition locations (not null) - LSP definition feature may be broken'
    );
  ...
  test('Completion returns suggestions', async function () {
    ...
    assert.ok(
      completions,
      'Should return completions (not null) - LSP completion feature may be broken'
    );
    ...
    assert.ok(
      ...
      'Completion item should have kind (CompletionItemKind)'
    );
  ...
  test('Class symbol appears in document symbols', async function () {
    ...
    if (testClassSymbol) {
      ...
      assert.ok(testClassSymbol.children !== undefined, 'Class symbol should have children array');
    }
  });
  ...
  test('Code lens shows reference counts and command is invocable', async function () {
    ...
    try {
      await vscode.commands.executeCommand('pike.showReferences', testDocumentUri.toString(), {
        line: funcPosition.line,
        character: funcPosition.character,
      });
      // Command executed successfully - verify we had references to show
      assert.ok(
        references!.length > 0,
        'pike.showReferences should open with references available'
      );
    } catch (err) {
  ...
  test('Code lens click with symbolName finds references even when position is at return type', async function () {
    ...
    assert.ok(
      references!.length >= 2,
      'Should find multiple references to test_function (definition + calls)'
    );
  });
  ...
  test('pike.showReferences command accepts symbolName parameter', async function () {
    ...
    assert.ok(
      references!.length >= 2,
      'test_function should have definition and at least one call'
    );
    ...
    try {
      ...
      assert.ok(
        references!.length >= 2,
        'pike.showReferences with symbolName should have references to display'
      );
    } catch (err) {
  ...
  test('Folding ranges have valid line numbers', async function () {
    ...
    for (const range of ranges) {
      ...
      assert.ok(
        range.end < lineCount,
        `Folding range end should be < ${lineCount}, got ${range.end}`
      );
      assert.ok(
        range.start <= range.end,
        `Folding range start (${range.start}) should be <= end (${range.end})`
      );
    }
  });
  ...
  test('Go-to-definition on module path does not crash', async function () {
    ...
    const locations = await vscode.commands.executeCommand<
      vscode.Location | vscode.Location[] | vscode.LocationLink[]
    >('vscode.executeDefinitionProvider', testDocumentUri, position);
  ...
  test('Hover provider does not crash on any position', async function () {
    ...
    for (const pos of positions) {
      ...
      assert.ok(
        hovers !== undefined,
        `Hover should not crash at position ${pos.line}:${pos.character}`
      );
    }
  });
...
suite('Waterfall Loading E2E Tests', () => {
  ...
  test('Completion shows stdlib import symbols', async function () {
    ...
    assertWithLogs(
      hasStdioSymbol,
      'Completion should show Stdio symbols due to import. Got: ' +
        completionLabels.slice(0, 10).join(', ')
    );
  });
  ...
  test('Test file has expected ModuleContext structure', async function () {
    ...
    assert.ok(
      text.includes('test_waterfall_completion'),
      'Should define "test_waterfall_completion"'
    );
    assert.ok(
      text.includes('test_stdlib_import_waterfall'),
      'Should define "test_stdlib_import_waterfall"'
    );
  });
  ...
  test('Roxen location module: File opens without crash', async function () {
    ...
    // File should open without causing LSP to crash
    assertWithLogs(
      locationDoc.languageId === 'pike',
      'Location module should be recognized as Pike'
    );
  });
