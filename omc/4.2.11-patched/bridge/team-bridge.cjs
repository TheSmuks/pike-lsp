...
[AUDIT WARNING: ${violations.length} permission violation(s) detected]`,
      ...
      const idleBackoffMax = config.idleBackoffMaxMs || 3e4;
      const idleBackoffMult = config.idleBackoffMultiplier || 2;
      const effectiveInterval = idleConsecutive > 0 ? Math.min(
...
function main() {
  ...
  config.idleBackoffMaxMs = config.idleBackoffMaxMs || 3e4;
  config.idleBackoffMultiplier = config.idleBackoffMultiplier || 2;
