...
async function main() {
  try {
    ...
    // TaskList rate limiting - enforce 15s minimum between calls
    if (toolName === 'TaskList') {
      const TASK_LIST_MIN_INTERVAL_MS = 15000;
      const statePath = join(directory, '.omc', 'state', 'tasklist-ratelimit.json');
      ...
      try {
        ...
        let state = { lastCallMs: 0 };
        if (existsSync(statePath)) {
          try {
            const content = readFileSync(statePath, 'utf-8');
            const parsed = JSON.parse(content);
            // Corruption guard: fail open if timestamp is future or stale (>1 hour)
            const now = Date.now();
            if (parsed.lastCallMs && parsed.lastCallMs <= now && (now - parsed.lastCallMs) <= 3600000) {
              state = parsed;
            }
        ...
        if (elapsed < TASK_LIST_MIN_INTERVAL_MS) {
          const waitSec = Math.ceil((TASK_LIST_MIN_INTERVAL_MS - elapsed) / 1000);
          console.log(JSON.stringify({
            decision: 'block',
            message: `TaskList rate-limited: called ${Math.round(elapsed/1000)}s ago (min: 15s). Wait ${waitSec}s. Process incoming teammate messages instead of polling.`
          }));
          return;
        }

        // Update timestamp
        writeFileSync(statePath, JSON.stringify({ lastCallMs: now }));
