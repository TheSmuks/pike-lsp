...
export async function runBridge(config: BridgeConfig): Promise<void> {
  ...
  let idleConsecutive = 0;  // Count of consecutive idle cycles for backoff
  ...
  while (true) {
    try {
      ...
      if (task) {
        idleNotified = false;
        idleConsecutive = 0;  // Reset backoff when task found
      ...
      // --- 12. Adaptive poll interval with backoff ---
      const idleBackoffMax = config.idleBackoffMaxMs || 30000;
      const idleBackoffMult = config.idleBackoffMultiplier || 2;
      const effectiveInterval = idleConsecutive > 0
        ? Math.min(
            config.pollIntervalMs * Math.pow(idleBackoffMult, Math.min(idleConsecutive, 10)),
            idleBackoffMax
          )
        : config.pollIntervalMs;

      // Wait in segments to stay responsive to shutdown/drain signals
      let waited = 0;
      while (waited < effectiveInterval) {
        const chunk = Math.min(config.pollIntervalMs, effectiveInterval - waited);
        await sleep(chunk);
        waited += chunk;

        // Check shutdown between wait segments
        const shutdownDuringWait = checkShutdownSignal(teamName, workerName);
        if (shutdownDuringWait) {
          await handleShutdown(config, shutdownDuringWait, activeChild);
          return;
        }
        // Check drain between wait segments
        const drainDuringWait = checkDrainSignal(teamName, workerName);
        if (drainDuringWait) {
          await handleShutdown(config, { requestId: drainDuringWait.requestId, reason: `drain: ${drainDuringWait.reason}` }, activeChild);
          return;
        }
      }
    } catch (err) {
